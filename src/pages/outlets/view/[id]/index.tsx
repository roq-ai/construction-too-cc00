...
import { deleteConstructionToolById } from 'apiSdk/construction-tools';
...
function OutletViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<OutletInterface>(
    ...
    () =>
      getOutletById(id, {
        relations: ['company', 'construction_tool'],
      }),
  );

  const construction_toolHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
              ...
              {hasAccess('construction_tool', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/construction-tools/create?outlet_id=${data?.id}`}>
                  ...
                  <TableContainer mb={4}>
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.construction_tool?.map((record) => (
                          <Tr
                            cursor="pointer"
                            onClick={() => router.push(`/construction-tools/view/${record.id}`)}
                            key={record.id}
                          >
                            <Td>{record.name}</Td>
                            <Td>{record.price}</Td>
                            ...
                            <Td>
                              {hasAccess(
                                ...
                                <NextLink
                                  onClick={(e) => e.stopPropagation()}
                                  passHref
                                  href={`/construction-tools/edit/${record.id}`}
                                >
                              ...
                              {hasAccess(
                                ...
                                <IconButton
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    construction_toolHandleDelete(record.id);
                                  }}
                                  colorScheme="red"
                                  variant="outline"
                                  aria-label="edit"
